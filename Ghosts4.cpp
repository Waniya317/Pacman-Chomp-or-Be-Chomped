...
bool isFrightened4 = false;

Ghosts4::Ghosts4(int startX, int startY) : x(startX), y(startY)
{
    this->startX = startX;
    this->startY = startY;
    ...
    for (int i = 0; i < 7; i++) {
        for (int j = 0; j < 7; j++) {
            if (i < 4 && (i - 3) * (i - 3) + (j - 3) * (j - 3) <= 9) {
                image.setPixel(j, i, sf::Color(255, 0, 0));
            }
            else if (i >= 4) {
                if ((j == 1 && i >= 5) || (j == 2 && i >= 4) ||
                    (j == 3 && i >= 5) || (j == 4 && i >= 4) ||
...
int Ghosts4::getX() { return x; }
int Ghosts4::getY() { return y; }

void Ghosts4::moves(char layout[maximum_rows][maximum_cols], int pacX, int pacY, int gx1, int gy1, int gx2, int gy2, int gx3, int gy3)
{
    int bestX = x;
    int bestY = y;

    int bestDist = isFrightened4 ? INT_MIN : INT_MAX;
...
void Ghosts4::setFrightened(bool value) {
    isFrightened4 = value;

    sf::Color ghostColor = value ? sf::Color(152, 255, 152) : sf::Color(255, 0, 0);
...
void Ghosts4::draw(RenderWindow& window)
{
    // Re-center smaller ghost in the tile
    float scaleFactor = 1.4f;
